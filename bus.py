...
# 
# There are three algorithms for finding the arrival estimates at
# a particular stop.
#
# 1. If the request includes a specific route, we can go directly to
#    a specific web page to retreive the estimates. That URL is stored
#    in the datastore.
#
# 2. We start a collection of background tasks to go off and fetch
#    scheduling data for each route that travels through the respective
#    stop. The results of those tasks are aggregated to determine the
#    best results.
#
# 3. We aggregate the scheduling data for every route that travels through 
#    the respective stop. However, the results are retrieved asynchronously
#    on the main request thread. This algorithm has limits, however. The
#    entire request needs to be serviced in less than 30 seconds.
#
...
def findBusAtStop(routeID, stopID):
      ...
      if len(routeQuery) > 0:
          ...
          smsBody = "Route %s, Stop %s: " % (routeID, stopID) + "Sorry. We do not have this route information on file." 
          return(smsBody)
